...
async function main() {
  ...
  const rewardRouter = await contractAt(
    "RewardRouter",
    "0x7B01aCf6e7e9CC276e644ac65D770c1131583453"
  );
  const stakedGmxTracker = await contractAt(
    "RewardTracker",
    "0x908C4D94D34924765f1eDc22A1DD098397c59dD4"
  );
  const bonusGmxTracker = await contractAt(
    "RewardTracker",
  ...
  for (let i = 0; i < stakeGmxList.length; i++) {
    ...
    console.log(
      `${i} ${address}: ${formatEther(balance)}, ${formatEther(stakedAmount)}`
    );

    if (!stakedAmount.eq(balance)) {
      throw new Error(
        `Invalid stakedAmount: ${address}, ${formatEther(
          balance
        )}, ${formatEther(stakedAmount).toString()}`
      );
    }
    ...
    console.log(
      `${address}: ${formatEther(pendingRewards).toString()}, ${formatEther(
        pendingBonus
      ).toString()}`
    );
