...
describe("Vault.settings", function () {
  ...
  it("setFundingRate", async () => {
    await expect(
      vault.connect(user0).setFundingRate(59 * 60, 10001000, 10001000)
    ).to.be.revertedWith("Vault: forbidden");

    await vault.setGov(user0.address);

    await expect(
      vault.connect(user0).setFundingRate(60 * 60, 10001000, 10001000)
    ).to.be.revertedWith("Vault: invalid _fundingRateFactor");

    await expect(
      vault.connect(user0).setFundingRate(60 * 60, 10000000, 10001000)
    ).to.be.revertedWith("Vault: invalid _stableFundingRateFactor");

    expect(await vault.fundingInterval()).eq(8 * 60 * 60);
    expect(await vault.fundingRateFactor()).eq(600000);
    expect(await vault.stableFundingRateFactor()).eq(600000);
    await vault.connect(user0).setFundingRate(60 * 60, 10000000, 10000000);
    expect(await vault.fundingInterval()).eq(60 * 60);
    expect(await vault.fundingRateFactor()).eq(10000000);
    expect(await vault.stableFundingRateFactor()).eq(10000000);

    await vault.connect(user0).setFundingRate(120 * 60, 1000000, 2000000);
    expect(await vault.fundingInterval()).eq(120 * 60);
    expect(await vault.fundingRateFactor()).eq(1000000);
    expect(await vault.stableFundingRateFactor()).eq(2000000);
  });
  ...
  it("setUsdgAmount", async () => {
    ...
    await expect(
      vault.connect(user0).setUsdgAmount(bnb.address, 50000)
    ).to.be.revertedWith("Vault: forbidden");
